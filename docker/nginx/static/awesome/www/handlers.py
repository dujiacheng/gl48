...
@get('/manage/') #直接跳到评论管理页面, 这也没有被用到吧..
def manage():
    return 'redirect:/manage/comments'


@get('/manage/comments') #du 已看
...
@post('/api/blogs/{id}/comments') #du 已看
...
@post('/api/comments/{id}/delete') #du 已看
...
@get('/show_all_users') #只是一个api,没有在页面上展示.
def show_all_users():
    # 显示所有的用户
...
def api_get_users(request):
    ...
    for u in users:
        u.passwd = '******'
    return dict(users=users) #完整版在上面, 已经被注释了.


@get('/manage/users') #du 已看
...
@get('/manage/blogs/create') #du 创建博客界面
...
@get('/manage/blogs')  #du已看
def manage_blogs(*, page='1'):
    # 博客管理页面
    return {
        '__template__': 'manage_blogs.html',
        'page_index': get_page_index(page),
        # 'dujiacheng': 'all right'
    }
...
@post('/api/blogs/{id}/delete') #du 已看
