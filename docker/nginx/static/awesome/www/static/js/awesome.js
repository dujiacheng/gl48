...
if (! Number.prototype.toDateTime) {
    ...
    Number.prototype.toDateTime = function(format) {
        ...
        var dt = new Date(this * 1000); //这里的this 就是blog.created_at
...
// ajax submit form:
// 'GET' 'api/blogs'  'Page=1' function (err, results) { initVM(results)};
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) { //看一下这个函数得不到返回的json会怎么样
        if (r && r.error) { 
...
function getJSON(url, data, callback) {
    ...
    if (arguments.length===2) { //如果只有两个参数, data就是回调函数,而不是data, 所以要进行赋值,互换位置.
        callback = data;
        data = {};
    }
    if (typeof (data)==='object') {
        // alert("typeof(data) is object")
        var arr = [];
        $.each(
         function (k, v) {
            // alert(k)
